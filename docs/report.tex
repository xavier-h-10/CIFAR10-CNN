...
\section{Appendix A}
在选择方向的过程中，我们回顾了自从CNN提出以来的各种模型以及其在数据集上的表现，并且获取了一些TensorFlow实现并且运行。您可以在./others里面查看这些模型并且复现结果。由于在部分模型中，数据对显存要求大于对显卡性能要求，我们没有保持硬件环境的严格一致，以求在花费尽可能低的情况下查看他们的效果。
\begin{table}[h]
	\centering
	\caption{各类模型的运行结果}
	\setlength{\tabcolsep}{0.6mm}{
		\begin{tabular}{l|l|l|l|l|l|l}
			\toprule
			Arch name   & Hardware            & Param size & Epochs & batch\_size & Running time & Accuracy \\ \midrule
