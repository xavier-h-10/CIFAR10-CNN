...
\subsubsection{优化器}
常见的优化器包括SGD, Adam, LAMB. 大部分的卷积神经网络都采用了SGD作为优化器，也有人采用Adam作为优化器，近几年的ResNet论文也有建议采用LAMB的。上述的Benchmark Top 1\cite{dosovitskiy2021image}论文中指出了在ResNet50模型中，各个优化器的表现是几乎没有任何差别的。根据一个DL科学家的回答\cite{QuoraAnswer}，使用SGD With momentum 因此，我们简单的采用阅读论文时的观察和建议，选择了SGD With momentum.
